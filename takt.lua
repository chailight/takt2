...
function init()
  ...
  math.randomseed(os.time())
    ...
    params:set_action('save_p', function(x) if pattern_name ~= nil then textentry.enter(save_project,  pattern_name) else textentry.enter(save_project,  'new') end)
...
function sequencer()
    ...
    while is_running do
        clock.sync(1/4) -- stay in sync with link quantum
        for j=1,32 do -- 
            --clock.sync(1/4)
